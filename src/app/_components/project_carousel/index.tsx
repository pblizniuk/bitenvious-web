  ...
  return (
    ...
    >
      {items && items.map((item) => {
        return (
          <SwiperSlide key={item.id} className='flex-auto justify-center items-center w-[50vw] md:w-[30vw] max-w-[465px] overflow-hidden'>
            <div key={item.id} className='group p-4 text-center'>
              ...
              <p className='text-lg font-light md:mt-4 md:text-xl'>{item.Description}</p>
            </div>
          </SwiperSlide>
        )
      })}
